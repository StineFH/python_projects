...
def softmax(X):
    ...
    ### YOUR CODE HERE
    row_max = np.amax(X, keepdims=True, axis=1)
    log_sum = np.log(np.sum(np.exp(X - row_max), keepdims=True, axis=1)) + row_max
    res = np.exp(X - log_sum)
    ### END CODE
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        bias1 = b1.reshape(1, b1.shape[1])
        Z1 = X@W1 + bias1
        activation = relu(Z1)
        bias2 = b2.reshape(1, b2.shape[1])
        Z2 = activation@W2 + bias2
        out = softmax(Z2)
    ...
    def score(self, X, y, params=None):
        ...
        ### YOUR CODE HERE
        acc = (self.predict(X, params) == y).mean()
        ### END CODE
    ...
    def cost_grad(X, y, params, c=0.0):
        ...
        loss_per_dp = -np.log(np.sum(labels*f, axis=1)) # labels*f Just multiplies each row with oppositie in the other matrix
        loss_mean = loss_per_dp.mean()
        reg = c * ((np.sum(np.square(W1))) + (np.sum(np.square(W2))))
        loss_reg = loss_mean + reg
        ...
        dl_df = -labels + f
        d_w2 = (np.dot(q.T, dl_df))/X.shape[0]
        d_b2 = np.mean(dl_df, axis=0, keepdims=True)
        dl_db = np.dot(dl_df, W2.T)*np.where(q<=0, 0, 1)
        d_w1 = (np.dot(X.T, dl_db))/X.shape[0]
        d_b1 = np.mean(dl_db, axis=0, keepdims = True)
        ...
        return loss_reg, {'d_w1': d_w1, 'd_w2': d_w2, 'd_b1': d_b1, 'd_b2': d_b2}
    ...
    def fit(self, X_train, y_train, X_val, y_val, init_params, batch_size=32, lr=0.1, c=1e-4, epochs=30):
        ...
        best_params = {'W1': None, 'b1': None, 'W2': None, 'b2': None}
        best_epoch = 0
        
        lst = list(range(X_train.shape[0]))
        for e in range(epochs):
            lst_permute = np.random.permutation(lst)
            for batch in range(0, X_train.shape[0], batch_size):
                
                if batch == range(0, X_train.shape[0], batch_size)[-1]:
                    X_chosen = X_train[lst_permute[batch:],]
                    Y_chosen = y_train[lst_permute[batch:]]
                    _, g_dic = self.cost_grad(X_chosen, Y_chosen, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}, c)
                    W1 = W1 - lr*g_dic['d_w1']
                    b1 = b1 - lr*g_dic['d_b1']
                    W2 = W2 - lr*g_dic['d_w2']
                    b2 = b2 - lr*g_dic['d_b2']
                else:
                    X_chosen = X_train[lst_permute[batch:batch+batch_size-1],] 
                    Y_chosen = y_train[lst_permute[batch:batch+batch_size-1]]
                    _, g_dic = self.cost_grad(X_chosen, Y_chosen, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}, c)
                    W1 = W1 - lr*g_dic['d_w1']
                    b1 = b1 - lr*g_dic['d_b1']
                    W2 = W2 - lr*g_dic['d_w2']
            ...
            train_loss[e], _ = self.cost_grad(X_train, y_train, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}, c)
            val_loss[e], _ = self.cost_grad(X_val, y_val, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}, 0)
            
            train_score = self.score(X_train, y_train, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2})
            val_score = self.score(X_val, y_val, {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2})
            
            train_acc[e] = train_score
            val_acc[e] = val_score
